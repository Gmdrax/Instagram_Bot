(() => {
    ...
    // ==========================================
    // 💀 PHANTOM CONFIG (V10.0 CHIMERA - HUMAN EMULATOR)
    // ==========================================
    const CFG = {
        HASH_FOLLOWERS: "c76146de99bb02f6415203be841dd25a", HASH_FOLLOWING: "3dec7e2c57367ef3da3d987d89f9dbc8", APP_ID: "936619743392459", 
        TIMINGS: { SCAN_BASE: 1200, ACTION_BASE: 5000, DM_BASE: 9000, BATCH_PAUSE: 300000, MACRO_PAUSE: 3600000 },
    ...
    // ==========================================
    // 🛡️ UTILS & BIOMETRÍA NEUROMOTORA
    // ==========================================
    const Utils = {
        sanitize: s => typeof s === 'string' ? Object.assign(document.createElement('div'), {textContent: s}).innerHTML : '',
        sleep: ms => new Promise(r => setTimeout(r, ms)),
        fmt: ms => ms >= 3600000 ? `${Math.floor(ms/3600000)}h ${Math.floor((ms%3600000)/60000)}m` : ms >= 60000 ? `${Math.floor(ms/60000)}m ${Math.floor((ms%60000)/1000)}s` : `${Math.ceil(ms/1000)}s`,
        // Fluctuación neuromotora (Simulación de cansancio humano)
        getFatigueDelay: (a, b) => {
            const fatigue = 1 + (a * 0.03); 
            const jitter = 0.8 + (Math.sin(Date.now()) * 0.4 + Math.random() * 0.3); 
            return Math.floor(b * fatigue * jitter);
        },
    ...
    // ==========================================
    // 👻 GHOST ENGINE V10.0 (DOM RENDERING & BANZAI)
    // ==========================================
    const Ghost = {
        scrollInterval: null,
        ...
        startHumanNoise() {
            ...
            // 1. Ruido Físico (Simulación de Hardware Inercial)
            this.scrollInterval = setInterval(() => {
                if (Store.state.status !== "run") return;
                window.scrollBy({ top: (Math.random() > 0.5 ? 1 : -1) * (Math.random() * 600), behavior: 'smooth' });
                const ev = new MouseEvent('mousemove', { bubbles: true, cancelable: true, clientX: Math.random() * window.innerWidth, clientY: Math.random() * window.innerHeight });
                document.dispatchEvent(ev);
            }, 3500 + Math.random() * 4000);
            
            // 2. Latidos de Red Constantes (Sync / Comet Simulator)
            this.networkInterval = setInterval(async () => {
                if (Store.state.status !== "run") return;
                try { await Net.req(`https://www.instagram.com/api/v1/feed/reels_tray/`, "GET"); } catch(e){}
            }, 22000 + Math.random() * 18000); 

            // 3. V10.0: Banzai Spoof Avanzado (time_spent y navigation logs)
            this.banzaiInterval = setInterval(async () => {
                if (Store.state.status !== "run") return;
                try {
                    const bz = new URLSearchParams();
                    const timeSpent = Math.floor(Math.random() * 15000) + 5000;
                    // Simulamos que pasamos X milisegundos mirando la pantalla
                    const payload = `[{"app_id":"${CFG.APP_ID}","posts":[["time_spent",{"event_time":${Date.now()},"tos_id":"${Utils.getUUID()}","time_spent":${timeSpent},"module":"profile"}]]}]`;
                    bz.append('q', payload);
                    bz.append('ts', Date.now().toString());
                    await Net.req(`https://www.instagram.com/ajax/bz?__a=1`, "POST", bz.toString());
                } catch(e){}
        ...
        stopHumanNoise() {
            ...
            if(this.networkInterval) { clearInterval(this.networkInterval); this.networkInterval = null; }
            if(this.banzaiInterval) { clearInterval(this.banzaiInterval); this.banzaiInterval = null; }
        },

        // 🧠 V10.0 MOTOR DE DESCARGA DOM NATIVA (Blindado contra fallos de API)
        async simulateDOMAssetLoading(mediaItems) {
            if (!mediaItems || !Array.isArray(mediaItems) || mediaItems.length === 0) return;
            const limit = Math.min(mediaItems.length, Math.floor(Math.random() * 4) + 2); 
            
            for (let i = 0; i < limit; i++) {
                try {
                    const item = mediaItems[i];
                    if (!item) continue;
                    let imgUrl = item?.image_versions2?.candidates?.[0]?.url || item?.node?.display_url || item?.display_url;
                    
                    if (imgUrl) {
                        const img = new Image();
                        img.src = imgUrl; 
                        await Utils.sleep(Math.random() * 400 + 150); 
                    }
                } catch(e) {
                    // Fallo silencioso si la imagen no existe
                }
            }
        },

        // ⌨️ V10.0 BIOMETRÍA DE ESCRITURA PARA DMs
        async simulateTypingDelay(textLength) {
            const msPerChar = 120 + Math.random() * 80; 
            const thinkingPause = Math.random() > 0.7 ? 2000 : 0; 
            const typingTime = (textLength * msPerChar) + thinkingPause;
            const finalTime = Math.min(typingTime, 15000);
            
            ErrorHandler.log(`⌨️ Escribiendo mensaje (${Utils.fmt(finalTime)})...`, "mut");
            await Utils.sleep(finalTime);
        },
        
        async preFlightProfileVisit(username, uid) {
            ErrorHandler.log(`👁️ Visitando perfil de @${username}...`, "mut");
            try { 
                await fetch(`/${username}/`, { headers: { "accept": "text/html" } }); 
                await Utils.sleep(1000 + Math.random() * 1500); 
                
                const feed = await Net.req(`https://www.instagram.com/api/v1/feed/user/${uid}/`);
                
                if (feed && feed.items) {
                    await this.simulateDOMAssetLoading(feed.items);
                }
                await Utils.sleep(1500 + Math.random() * 2000);
            } catch(e){}
        },
        
        async postActionWander() {
            ErrorHandler.log(`🏠 Volviendo al Inicio (Mirando Feed)...`, "mut");
            try {
                const tl = await Net.req(`https://www.instagram.com/api/v1/feed/timeline/`, "POST", "reason=pull_to_refresh");
                if (tl && !tl.err && Array.isArray(tl.feed_items)) {
                    await this.simulateDOMAssetLoading(tl.feed_items.map(i => i?.media_or_ad).filter(Boolean));
                }
                await Utils.sleep(3000 + Math.random() * 4000); 
                
                if (Math.random() < 0.35) {
                    ErrorHandler.log(`🌍 Pestaña Explorar (Distracción)...`, "mut");
                    const exp = await Net.req(`https://www.instagram.com/api/v1/discover/topical_explore/`);
                    if (exp && !exp.err) {
                        let medias = [];
                        // Instagram actualiza constantemente esta estructura, buscamos en ambas posibles
                        if (Array.isArray(exp.sectional_items)) {
                            medias = exp.sectional_items.map(i => i?.layout_content?.medias?.[0]?.media).filter(Boolean);
                        } else if (Array.isArray(exp.items)) {
                            medias = exp.items.map(i => i?.media).filter(Boolean);
                        }
                        
                        if (medias.length > 0) {
                            await this.simulateDOMAssetLoading(medias);
                        }
                    }
                    await Utils.sleep(2500 + Math.random() * 3000);
                }
            } catch(e){
                // Aislamos completamente esta sección. Si estalla por la API de IG, el loop principal de DMs/Follows no se entera.
            }
        }
    };
    ...
    const DB = {
        db: null,
        init() { return new Promise((res, rej) => { const req = indexedDB.open("PhantomV10_Core", 1); req.onupgradeneeded = e => e.target.result.createObjectStore("processed", { keyPath: "id" }); req.onsuccess = e => { this.db = e.target.result; res(); }; req.onerror = rej; }); },
    ...
    const Engine = {
        async init() { try { await DB.init(); ErrorHandler.log("💽 Motor V10.0 CHIMERA Cargado. Emulador humano online.", "suc"); } catch (e) { ErrorHandler.handle(e, "Arranque DB"); } },
        selfDestruct() { Store.state.status = "dead"; Net.abort(); Ghost.stopHumanNoise(); EventBus.emit('SELF_DESTRUCT'); },
        async runScan() {
        ...
        async engage() {
            ...
            if (Store.state.status !== "run") {
                Store.state.stats = { ...Store.state.stats, success: 0, fail: 0, errs: 0, backoff: 30000 };
            }
            Store.state.status = "run"; Net.resetController();
            ...
            ErrorHandler.log(`🚀 Iniciando CHIMERA Protocol: ${Store.state.mode.toUpperCase()}`, "info");
            Ghost.startHumanNoise(); 
            ...
            try {
                for (let i = 0; i < targets.length; i++) {
                    ...
                    if (Store.state.stats.success >= sLimit) { 
                        Ghost.stopHumanNoise(); 
                        const mPause = CFG.TIMINGS.MACRO_PAUSE + Math.floor(Math.random() * 900000);
                        ErrorHandler.log(`⏳ Sesión completada. Durmiendo ${Utils.fmt(mPause)} para purgar logs de IG...`, "wait"); 
                        
                        let el = 0; 
                        while (el < mPause && Store.state.status === "run") { 
                            await Utils.sleep(1000); el += 1000; EventBus.emit('TIMER_UPDATED', mPause - Math.min(el, mPause)); 
                        }
                        if (Store.state.status !== "run") break; 
                        Store.state.stats = { ...Store.state.stats, success: 0 }; 
                        Ghost.startHumanNoise();
                        ErrorHandler.log(`🚀 Despertando. Reanudando lista...`, "suc");
                    }
                    
                    const uid = targets[i], user = Store.pool.get(uid); let ok = false, aIcon = "✔️";

                    // GHOST CHIMERA: Visita de perfil + Descarga DOM Nativa
                    await Ghost.preFlightProfileVisit(user.username, uid);
                    ...
                    if (Store.state.mode === "boost_feed") {
                        ...
                        else { const pId = s.pk || s.id?.split('_')[0]; if(!pId){ok=true; aIcon="⚠️ (ID roto)";} else { const r = await Net.req(`https://www.instagram.com/api/v1/story_interactions/send_story_like/`, "POST", `media_id=${pId}`); ok = r?.status === "ok" ? (aIcon="🔥", true) : r||{err:"UNKNOWN"}; } }
                    } else if (Store.state.mode === "message") {
                        const m = Utils.spinText(Store.state.dmMessage.replace(/@usuario/gi, user.username));
                        
                        // 🔥 CHIMERA: Simulación de tipeo biológico antes de enviar el paquete de texto
                        await Ghost.simulateTypingDelay(m.length);

                        const uuid = Utils.getUUID();
                        const offId = Date.now().toString() + Math.floor(Math.random() * 999).toString().padStart(3, '0');
                        const p = new URLSearchParams(); p.append('recipient_users', `[["${uid}"]]`); p.append('action', 'send_item'); p.append('is_shh_mode', '0'); p.append('send_attribution', 'direct_thread'); p.append('client_context', uuid); p.append('offline_threading_id', offId); p.append('mutation_token', offId); p.append('text', m);
                    ...
                    if (ok === true) {
                        ...
                        Store.toggleQueue(uid, false); DB.save(uid); EventBus.emit('ITEM_PROCESSED', { id: uid, result: 'ok' }); ErrorHandler.log(`${aIcon} @${user.username}`, "suc");
                    } else {
    ...
    (async () => { UI.init(); await Engine.init(); ErrorHandler.log("🧬 V10.0 CHIMERA. Emulación Humana Total Iniciada.", "suc"); })();
})();