(() => {
    ...
    // ==========================================
    // 💀 PHANTOM CONFIG (V9.5 APEX STEALTH - THE MASTERPIECE)
    // ==========================================
    const CFG = {
    ...
    // Motor de Utilidades con Retraso Biométrico (Log-Normal Simulation)
    const Utils = {
        ...
        // Fluctuación neuromotora (no es un random plano)
        getFatigueDelay: (a, b) => {
    ...
    // ==========================================
    // 👻 GHOST ENGINE V9.5: APEX (ASSET SPOOFING + BANZAI)
    // ==========================================
    const Ghost = {
        scrollInterval: null,
        ...
        startHumanNoise() {
            ...
            // 1. Ruido Físico (Simulación de Hardware)
            ...
            // 3. Falsificación de Analíticas Banzai Exactas al .HAR
            this.banzaiInterval = setInterval(async () => {
                if (Store.state.status !== "run") return;
                try {
                    const bz = new URLSearchParams();
                    bz.append('q', `[{"app_id":"${CFG.APP_ID}","posts":[["falco_js_error",{"event_time":${Date.now()},"metadata":"{\\"mouse_x\\":${Math.floor(Math.random()*1000)},\\"mouse_y\\":${Math.floor(Math.random()*1000)},\\"scroll_depth\\":${Math.floor(Math.random()*3000)}}"}]]}]`);
        ...
        // 🖼️ MOTOR DE DESCARGA CIEGA (Simula la carga de CDN de un navegador real)
        async simulateAssetLoading(mediaItems) {
            if (!mediaItems || !mediaItems.length) return;
            const limit = Math.min(mediaItems.length, Math.floor(Math.random() * 4) + 2); // Carga entre 2 y 5 imágenes
            for (let i = 0; i < limit; i++) {
                const item = mediaItems[i];
                let imgUrl = item?.image_versions2?.candidates?.[0]?.url || item?.node?.display_url;
                if (imgUrl) {
                    try {
                        // Modo no-cors evita bloqueos de seguridad del navegador, pero deja la huella de red en IG
                        fetch(imgUrl, { mode: 'no-cors', priority: 'low' }).catch(()=>{});
                        await Utils.sleep(Math.random() * 300 + 100); // 100-400ms de latencia de red simulada
                    } catch(e) {}
                }
            }
        },
        ...
        async preFlightProfileVisit(username, uid) {
            ErrorHandler.log(`👁️ Perfil @${username} (Renderizando assets)...`, "mut");
            try { 
                // Carga la shell HTML
                await fetch(`/${username}/`, { headers: { "accept": "text/html" } }); 
                await Utils.sleep(800 + Math.random() * 1000); 
                
                // Carga el feed de fotos (JSON)
                const feed = await Net.req(`https://www.instagram.com/api/v1/feed/user/${uid}/`);
                
                // Extrae las URLs reales de las fotos y las descarga (APEX STEALTH)
                if (feed && feed.items) {
                    await this.simulateAssetLoading(feed.items);
                }
        ...
        async postActionWander() {
            ErrorHandler.log(`🏠 Scroll en Home Feed...`, "mut");
            try {
                const tl = await Net.req(`https://www.instagram.com/api/v1/feed/timeline/`, "POST", "reason=pull_to_refresh");
                if (tl && tl.feed_items) await this.simulateAssetLoading(tl.feed_items.map(i => i.media_or_ad).filter(Boolean));
                await Utils.sleep(3000 + Math.random() * 4000); 
                
                if (Math.random() < 0.35) {
                    ErrorHandler.log(`🌍 Pestaña Explorar...`, "mut");
                    const exp = await Net.req(`https://www.instagram.com/api/v1/discover/topical_explore/`);
                    if (exp && exp.sectional_items) await this.simulateAssetLoading(exp.sectional_items.map(i => i.layout_content?.medias?.[0]?.media).filter(Boolean));
    ...
    const DB = {
        db: null,
        init() { return new Promise((res, rej) => { const req = indexedDB.open("PhantomV9_Core", 1); req.onupgradeneeded = e => e.target.result.createObjectStore("processed", { keyPath: "id" }); req.onsuccess = e => { this.db = e.target.result; res(); }; req.onerror = rej; }); },
    ...
    const Engine = {
        async init() { try { await DB.init(); ErrorHandler.log("💽 Motor V9.5 APEX Cargado. Simulación Biológica Activa.", "suc"); } catch (e) { ErrorHandler.handle(e, "Arranque DB"); } },
        ...
        async engage() {
            ...
            ErrorHandler.log(`🚀 Iniciando APEX Protocol: ${Store.state.mode.toUpperCase()}`, "info");
            ...
            try {
                for (let i = 0; i < targets.length; i++) {
                    ...
                    // GHOST APEX: Visita de perfil + Descarga ciega de imágenes
                    ...
                    // GHOST APEX: Retorno al inicio y recarga del feed visual
    ...
    (async () => { UI.init(); await Engine.init(); ErrorHandler.log("🥷 V9.5 APEX STEALTH INICIADO. Extracción ciega de red activa.", "suc"); })();
})();