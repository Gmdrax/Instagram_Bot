(() => {
    ...
    // ==========================================
    // 💀 PHANTOM CONFIG (V6.4 GOD MODE + LA PURGA)
    // ==========================================
    const CFG = {
        HASH_FOLLOWERS: "c76146de99bb02f6415203be841dd25a", 
        HASH_FOLLOWING: "3dec7e2c57367ef3da3d987d89f9dbc8", 
        HASH_LIKERS: "d5d763b1e2acf209d62d22d184488e57", // Hash para extraer Likes
    ...
    const Net = {
        ...
        async action(id, type) {
            ...
            return (r && ["ok", "following", "unfollowed", "removed"].includes(r.status || r.result)) ? true : { err: "UNKNOWN" };
        }
    };
    ...
    const Engine = {
        ...
        // Extractor de Interacciones (Antifantasmas)
        async gatherActiveUsers(targetUsername) {
            ErrorHandler.log("🕵️ Analizando interacciones profundas (Últimos 12 posts)...", "wait");
            const r = await Net.req(`https://www.instagram.com/api/v1/users/web_profile_info/?username=${targetUsername}`);
            
            // Expandido de 3 a 12 posts para no ser tan agresivo (Purga Suave)
            ...
            for (const p of posts) {
                ...
                await Utils.sleep(1500); // Pausa táctica entre llamadas
            }
            return activeIds;
        },
        ...
        async runScan() {
            ...
            ErrorHandler.log(`🔍 Escaneando V6.4 [${Store.state.mode.toUpperCase()}]: @${Store.state.target}...`);
            try {
                const id = await Net.resolve(Store.state.target);
                if (!id) throw new Error("Objetivo no encontrado.");

                let activeIds = new Set();
                if (Store.state.mode === "purga") {
                    activeIds = await this.gatherActiveUsers(Store.state.target);
                    ErrorHandler.log(`🛡️ ${activeIds.size} usuarios leales/activos detectados. Aislado fantasmas...`, "suc");
                }
                ...
                while (next && Store.state.status === "scan") {
                    ...
                    if (d?.err) throw new Error("Rate Limit en escáner.");
                    ...
                    for (const e of edge.edges) {
                        ...
                        // --- MEJORA: PURGA SUAVE E INTELIGENTE ---
                        if (Store.state.mode === "purga") {
                            if (activeIds.has(u.id)) continue; // Se salva: Interactuó en los últimos 12 posts
                            if (u.followed_by_viewer) continue; // Se salva: Es tu amigo/mutuo (tú lo sigues también)
                        }
                        
                        if (CFG.FILTERS.SKIP_VERIFIED && u.is_verified) continue; // Se salvan: Cuentas verificadas
                ...
                ErrorHandler.log(`✅ Escaneo seguro listo: ${Store.pool.size} perfiles ${Store.state.mode === "purga" ? "fantasmas" : "objetivos"}.`, "suc");
        ...
        async engage() {
            ...
            try {
                for (let i = 0; i < targets.length; i++) {
                    ...
                    const actionType = Store.state.mode === "purga" ? "remove_follower" : Store.state.mode;
                    const ok = await Net.action(uid, actionType);
                    ...
                    if (ok === true) {
                        ...
                        const icon = Store.state.mode === "follow" ? '➕' : Store.state.mode === "unfollow" ? '➖' : '👻';
    ...
    const UI = {
        ...
        bindEvents() {
            ...
            // Nuevo rotador de 3 Modos
            this.dom.mod.onclick = () => {
                const modes = ["follow", "unfollow", "purga"];
                Store.state.mode = modes[(modes.indexOf(Store.state.mode) + 1) % modes.length];
            };
    ...
    // 🚀 BOOT SEQUENCE
    // ==========================================
    (async () => { UI.init(); await Engine.init(); ErrorHandler.log("⚡ V6.4 GOD MODE ONLINE. Protocolo Purga Listo.", "suc"); })();
})();